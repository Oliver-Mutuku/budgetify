...
class IncomeFilter(django_filters.FilterSet):
    filter_by = django_filters.CharFilter(method='filter_by_date_range')
    start = django_filters.DateFilter(field_name='date_of_income', lookup_expr='gte')
    end = django_filters.DateFilter(field_name='date_of_income', lookup_expr='lte')

    class Meta:
        model = Income
        fields = ['filter_by', 'start', 'end', 'source']
    ...
    def filter_by_date_range(self, queryset, name, value):
        ...
        if value == 'past_week':
            ...
            return queryset
        return queryset.filter(date_of_income__gte=start_date)
